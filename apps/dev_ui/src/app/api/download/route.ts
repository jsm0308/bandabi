...
import { resolveRunFilePath } from "../../../lib/runsRepo";
...
export async function GET(req: NextRequest) {
  ...
  // 허용 파일만 (안전)
  const allowed = new Set(["leaderboard.csv", "metrics.csv", "routes.csv", "events.csv", "config_resolved.yaml"]);
  ...
  headers.set("Content-Disposition", `attachment; filename="${path.basename(file)}"`);

...
function bestVariant(rows: any[]) {
  const scored = rows
    .map((r) => ({ variant: r.variant, a: Number(r.center_late_p95), b: Number(r.vehicles_used) }))
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        ...
        <div className="flex gap-3">
          ...
          {best && (
            <Link className="text-sm text-neutral-300 hover:underline" href={`/experiments/${encodeURIComponent(expId)}/variants/${encodeURIComponent(best)}`}>
              Best → {best}
            </Link>
          )}
        </div>
